...
style type ="text /css ">a, a:hover {
  ...
  .card {
    ...
    box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.3);
    padding: 20px;
    margin: 80px auto; /* 가운데 정렬을 위해 margin 수정 */
    font-family: 'WooridaumB', sans-serif; /* 글꼴 설정 추가 */
  }
  ...
  .input-group {
    display: flex;
    align-items: center;
    margin-bottom: 10px; /* 간격을 10px로 수정 */
  }
  ...
  .submit-button {
    width: 100px;
    height: 40px;
    background-color: #4CAF50; /* 배경색을 #4CAF50로 수정 */
    border: none;
    border-radius: 4px;
    color: #ffffff; /* 글자색을 하양(#ffffff)로 수정 */
    font-size: 14px;
    font-weight: bold;
    cursor: pointer;
    margin-top: 20px; /* 버튼 위쪽 간격을 20px로 수정 */
    font-family: 'WooridaumB', sans-serif; /* 글꼴 설정 추가 */
  }
</style>
<meta charset="EUC-KR">
...
<div class="card">
    ...
    <form action="updateUserInfoProcess.jsp" method="post">
        <div class="input-group">
            ...
            <button type="submit" name="action" value="passwordCheck">비밀번호 확인</button>
        </div>
    </form>
    <br>
    <c:choose>
...
<script>
    document.getElementById('currentPassword').addEventListener('change', function() {
        var userInfoForm = document.getElementById('userInfoForm');
        var errorMessage = document.getElementById('errorMessage');
        userInfoForm.style.display = (this.value !== '') ? 'block' : 'none';
        errorMessage.style.display = 'none'; // 비밀번호 일치 여부 메시지 숨기기
    });
</script>
